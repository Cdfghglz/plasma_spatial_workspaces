...
MouseArea {
    ...
    // PSW: per-activity spatial grid state from KWin D-Bus
    property int spatialRows: 0
    property int spatialCols: 0
    property var spatialGridCells: []   // flat array of x11 desktop numbers, row-major; 0 = empty cell
    ...
    // PSW: Check whether a desktop (by pager model index) belongs to
    // the current activity's spatial map.  Returns false when spatial
    // mode is active and the desktop's x11 number is absent from the
    // grid cells array — those desktops live on other activities.
    function isDesktopInSpatialMap(modelIndex) {
        if (spatialGridCells.length === 0) return true  // no spatial data yet — show all
        var desktopNum = modelIndex + 1
        for (var i = 0; i < spatialGridCells.length; i++) {
            if (spatialGridCells[i] === desktopNum) return true
        }
        return false
    }

    // PSW: Compute grid column for a desktop by its pager model index.
    // Uses spatial grid layout when available, falls back to sequential fill.
    function spatialColumnForDesktop(modelIndex) {
        var desktopNum = modelIndex + 1
        if (spatialGridCells.length > 0 && spatialCols > 0) {
            for (var i = 0; i < spatialGridCells.length; i++) {
                if (spatialGridCells[i] === desktopNum) {
                    return i % spatialCols
                }
            }
        }
        return modelIndex % pagerItemGrid.effectiveColumns
    }

    // PSW: Compute grid row for a desktop by its pager model index.
    function spatialRowForDesktop(modelIndex) {
        var desktopNum = modelIndex + 1
        if (spatialGridCells.length > 0 && spatialCols > 0) {
            for (var i = 0; i < spatialGridCells.length; i++) {
                if (spatialGridCells[i] === desktopNum) {
                    return Math.floor(i / spatialCols)
                }
            }
        }
        return Math.floor(modelIndex / pagerItemGrid.effectiveColumns)
    }
    ...
    // PSW: Query KWin for per-activity spatial grid layout via D-Bus.
    // Each query appends a unique suffix (bash comment) so the executable
    // data engine doesn't return stale cached results.
    PlasmaCore.DataSource {
        id: spatialGridSource
        engine: "executable"
        property int querySeq: 0
        onNewData: {
            var stdout = (data["stdout"] || "").trim()
            disconnectSource(sourceName)
            if (sourceName.indexOf("spatialGridRows") !== -1) {
                root.spatialRows = parseInt(stdout) || 0
                if (root.spatialRows > 0) {
                    querySeq++
                    connectSource("qdbus org.kde.KWin /VirtualDesktopManager org.kde.KWin.VirtualDesktopManager.spatialGridColumns #" + querySeq)
                }
            } else if (sourceName.indexOf("spatialGridColumns") !== -1) {
                root.spatialCols = parseInt(stdout) || 0
                if (root.spatialCols > 0) {
                    querySeq++
                    connectSource("qdbus org.kde.KWin /VirtualDesktopManager org.kde.KWin.VirtualDesktopManager.spatialGridLayout #" + querySeq)
                }
            } else if (sourceName.indexOf("spatialGridLayout") !== -1) {
                if (stdout.length > 0) {
                    root.spatialGridCells = stdout.split(",").map(function(s) { return parseInt(s) || 0; })
                } else {
                    root.spatialGridCells = []
                }
            }
        }
        function refresh() {
            querySeq++
            connectSource("qdbus org.kde.KWin /VirtualDesktopManager org.kde.KWin.VirtualDesktopManager.spatialGridRows #" + querySeq)
        }
    }

    // PSW: Poll spatial grid periodically to catch activity switches.
    Timer {
        id: spatialRefreshTimer
        interval: 2000
        running: root.spatialRows > 0
        repeat: true
        onTriggered: spatialGridSource.refresh()
    }
    ...
    // PSW: Changed from Grid to Item for absolute positioning.
    // In spatial mode, desktops are placed at their spatial grid coordinates.
    // In standard mode, desktops are placed sequentially (same as Grid).
    Item {
        id: pagerItemGrid

        z: 1

        property real spacing: PlasmaCore.Units.devicePixelRatio

        width: effectiveColumns * columnWidth + (effectiveColumns - 1) * spacing
        height: effectiveRows * rowHeight + (effectiveRows - 1) * spacing
        ...
        Repeater {
            ...
            PlasmaCore.ToolTipArea {
                ...
                // PSW: Hide desktops not in the current activity's spatial map
                visible: root.isDesktopInSpatialMap(index)

                // PSW: Position based on spatial grid coords instead of sequential fill
                x: root.spatialColumnForDesktop(index) * (pagerItemGrid.columnWidth + pagerItemGrid.spacing)
                y: root.spatialRowForDesktop(index) * (pagerItemGrid.rowHeight + pagerItemGrid.spacing)
    ...
    Component.onCompleted: {
        // PSW: initial spatial grid query
        spatialGridSource.refresh()
