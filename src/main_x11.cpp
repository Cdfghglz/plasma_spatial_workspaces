...
// PSW crash diagnostic: print backtrace on SIGSEGV
#include <execinfo.h>
#include <signal.h>
#include <unistd.h>
#include <fcntl.h>
static void psw_sigsegv_handler(int sig) {
    const char msg[] = "\n=== PSW CRASH BACKTRACE (signal ";
...
void ApplicationX11::crashHandler(int signal)
{
    ...
    // PSW: print backtrace to stderr AND to a log file
    {
        void *frames[64];
        int n = backtrace(frames, 64);
        const char hdr[] = "\n=== PSW CRASH BACKTRACE ===\n";
        ...
        const char end[] = "=== END BACKTRACE ===\n";
        write(STDERR_FILENO, end, sizeof(end)-1);
        // Also write to /tmp/kwin-psw-backtrace.log
        int fd = open("/tmp/kwin-psw-backtrace.log", O_WRONLY|O_CREAT|O_TRUNC, 0644);
        if (fd >= 0) {
...
{
    // PSW: install backtrace handler early, before KCrash overrides it
    signal(SIGSEGV, psw_sigsegv_handler);
    signal(SIGABRT, psw_sigsegv_handler);
