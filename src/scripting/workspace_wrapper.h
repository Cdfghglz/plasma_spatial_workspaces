...
class WorkspaceWrapper : public QObject
{
    ...
    /**
     * Whether spatial desktop navigation mode is active.
     * When true, navigation between desktops uses explicit neighbor relationships
     * rather than grid positions.
     * @see setSpatialMode
     * @see spatialMapChanged
     */
    Q_PROPERTY(bool spatialMode READ spatialMode WRITE setSpatialMode NOTIFY spatialMapChanged)
...
Q_SIGNALS:
    ...
    /**
     * Emitted whenever spatialMode changes or the spatial neighbor map is modified.
     * @see spatialMode
     */
...
#undef GETTERSETTERDEF
    ...
    /**
     * Returns whether spatial desktop navigation mode is active.
     * @see setSpatialMode
     */
    bool spatialMode() const;

    /**
     * Sets spatial desktop navigation mode.
     * @param enabled If true, desktop navigation uses explicit neighbor relationships.
     */
    void setSpatialMode(bool enabled);

    /**
     * Returns the x11 desktop number of the desktop above @p desktop in the spatial map,
     * or 0 if no neighbor is set.
     * @param desktop The x11 desktop number (1-based).
     */
    Q_SCRIPTABLE int spatialNeighborUp(int desktop) const;

    /**
     * Returns the x11 desktop number of the desktop below @p desktop in the spatial map,
     * or 0 if no neighbor is set.
     * @param desktop The x11 desktop number (1-based).
     */
    Q_SCRIPTABLE int spatialNeighborDown(int desktop) const;

    /**
     * Returns the x11 desktop number of the desktop to the left of @p desktop in the spatial map,
     * or 0 if no neighbor is set.
     * @param desktop The x11 desktop number (1-based).
     */
    Q_SCRIPTABLE int spatialNeighborLeft(int desktop) const;

    /**
     * Returns the x11 desktop number of the desktop to the right of @p desktop in the spatial map,
     * or 0 if no neighbor is set.
     * @param desktop The x11 desktop number (1-based).
     */
    Q_SCRIPTABLE int spatialNeighborRight(int desktop) const;

    /**
     * Sets the spatial neighbor of @p fromDesktop in the given @p direction to @p toDesktop.
     * Pass 0 as @p toDesktop to clear the neighbor in that direction.
     *
     * @param fromDesktop  The x11 desktop number (1-based) of the source desktop.
     * @param direction    The direction: 0=Above, 1=Below, 2=Left, 3=Right.
     * @param toDesktop    The x11 desktop number (1-based) of the target desktop, or 0 to clear.
     */
    Q_SCRIPTABLE void setSpatialNeighbor(int fromDesktop, int direction, int toDesktop);
