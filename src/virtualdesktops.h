...
/**
 * @brief Stores explicit spatial neighbor relationships between virtual desktops.
 *
 ...
 *
 * Neighbor entries are persisted as Spatial_<uuid>_<Direction> keys in the
 * kwinrc [Desktops] config section. Only explicitly set neighbors are stored;
 * absent entries cause navigation to fall back to grid-based lookup.
 */
class KWIN_EXPORT VirtualDesktopSpatialMap
{
public:
    /**
     * @brief Directional neighbor types.
     */
    ...
    /**
     * Sets the neighbor of @p desktopId in @p direction to @p neighborId.
     * Pass an empty string to clear the neighbor.
     */
    void setNeighbor(const QString &desktopId, Direction direction, const QString &neighborId);

    /**
     * @returns The neighbor desktop ID of @p desktopId in @p direction,
     *          or an empty string if no neighbor is set.
     */
    QString neighbor(const QString &desktopId, Direction direction) const;

    /**
     * Removes all neighbor entries for @p desktopId.
     * Should be called when a desktop is destroyed.
     */
    void removeDesktop(const QString &desktopId);

    /**
     * Loads spatial neighbor entries from @p group.
     * Keys are of the form Spatial_<uuid>_Above, Spatial_<uuid>_Below, etc.
     */
    void load(const KConfigGroup &group);

    /**
     * Saves spatial neighbor entries to @p group.
     * Keys are of the form Spatial_<uuid>_Above, Spatial_<uuid>_Below, etc.
     * Clears any stale entries for desktops not in @p knownIds.
     */
...
public:
    ...
    /**
     * @returns @c true if spatial neighbor map navigation is enabled.
     * @see setSpatialMode
     * @see spatialModeChanged
     */
     ...
     * When spatialMode is enabled, delegates to the spatial neighbor map.
     */
...
public Q_SLOTS:
    ...
    /**
     * Enables or disables spatial neighbor map navigation.
     * @see isSpatialMode
     * @see spatialModeChanged
     */
...
inline
bool VirtualDesktopManager::isSpatialMode() const
{
