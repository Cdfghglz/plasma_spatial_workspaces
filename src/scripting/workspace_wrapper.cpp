...
WorkspaceWrapper::WorkspaceWrapper(QObject* parent) : QObject(parent)
{
    ...
    connect(vds, &VirtualDesktopManager::desktopNameChanged, this, &WorkspaceWrapper::desktopNameChanged);
...
bool WorkspaceWrapper::spatialMode() const
{
    return VirtualDesktopManager::self()->isSpatialMode();
}

void WorkspaceWrapper::setSpatialMode(bool enabled)
{
    VirtualDesktopManager::self()->setSpatialMode(enabled);
}

static int spatialNeighborInDirection(int desktop, VirtualDesktopSpatialMap::Direction direction)
{
    ...
    const QString neighborId = vds->spatialMap().neighbor(vd->id(), direction);
    if (neighborId.isEmpty()) {
        return 0;
    }
    const VirtualDesktop *neighbor = vds->desktopForId(neighborId);
    return neighbor ? static_cast<int>(neighbor->x11DesktopNumber()) : 0;
}

int WorkspaceWrapper::spatialNeighborUp(int desktop) const
{
    return spatialNeighborInDirection(desktop, VirtualDesktopSpatialMap::Direction::Above);
}

int WorkspaceWrapper::spatialNeighborDown(int desktop) const
{
    return spatialNeighborInDirection(desktop, VirtualDesktopSpatialMap::Direction::Below);
}

int WorkspaceWrapper::spatialNeighborLeft(int desktop) const
{
    return spatialNeighborInDirection(desktop, VirtualDesktopSpatialMap::Direction::Left);
}

int WorkspaceWrapper::spatialNeighborRight(int desktop) const
{
    return spatialNeighborInDirection(desktop, VirtualDesktopSpatialMap::Direction::Right);
}

void WorkspaceWrapper::setSpatialNeighbor(int fromDesktop, int direction, int toDesktop)
{
    VirtualDesktopManager *vds = VirtualDesktopManager::self();
    const VirtualDesktop *from = vds->desktopForX11Id(fromDesktop);
    if (!from) {
        return;
    }
    ...
    switch (direction) {
    case 0: dir = VirtualDesktopSpatialMap::Direction::Above; break;
    case 1: dir = VirtualDesktopSpatialMap::Direction::Below; break;
    case 2: dir = VirtualDesktopSpatialMap::Direction::Left;  break;
    case 3: dir = VirtualDesktopSpatialMap::Direction::Right; break;
    default: return;
    }

    QString toId;
    if (toDesktop != 0) {
        const VirtualDesktop *to = vds->desktopForX11Id(toDesktop);
        if (!to) {
            return;
        }
        toId = to->id();
    ...
    vds->spatialMap().setNeighbor(from->id(), dir, toId);
